...
class PrimitiveSetTyped(object):
    ...
    def addEphemeralConstant(self, name, ephemeral, ret_type):
        ...
        if name not in module_gp:
            class_ = type(
                name, (Ephemeral,), {"func": staticmethod(ephemeral), "ret": ret_type}
            )
            ...
            if issubclass(class_, Ephemeral):
                ...
                raise Exception(
                    "Ephemerals should be named differently "
...
def quick_evaluate_subtree(expr: PrimitiveTree, pset, data, prefix="ARG"):
    result = None
    stack = []
    result_subtree = []
    for id, node in enumerate(expr):
        ...
        while len(stack[-1][1]) == stack[-1][0].arity:
            ...
            if isinstance(prim, gp.Primitive):
                result = pset.context[prim.name](*args)
                result_subtree.append((id, result))
            elif isinstance(prim, gp.Terminal):
                if prefix in prim.name:
                    result = data[:, int(prim.name.replace(prefix, ""))]
                else:
            ...
            if len(stack) == 0:
                break  # If stack is empty, all nodes should have been seen
            stack[-1][1].append(result)
    return result, result_subtree


def pad_vector(vec, target_length):
    # 使用 np.atleast_1d 确保 vec 至少是一维数组
    vec = np.atleast_1d(vec)
    current_length = vec.shape[0]
    pad_width = target_length - current_length
    if pad_width <= 0:
        return vec[:target_length]
    return np.pad(vec, (0, pad_width), mode='constant', constant_values=0)

def SSC(ind1, ind2, y, pset, X):
    if len(ind1) < 2 or len(ind2) < 2:
        # No crossover on single node tree
        return ind1, ind2
    ...
    if len(common_types) > 0:
        ...
        similarity = []
        # R=False
        # for i in types2[type_]:
        #     # subtree = ind2[ind2.searchSubtree(i)]
        #     # y_pred_ground_truth = quick_evaluate(subtree, pset, X)
        #     y_pred = ind2.subtree_semantics[i]
        #     # assert np.allclose(y_pred, y_pred_ground_truth), (
        #     #     f"{y_pred[:10]} {y_pred_ground_truth[:10]}"
        #     # )
        #     try:
        #         similarity.append(
        #             cosine_similarity(y_pred.reshape(-1, 1), y.reshape(-1, 1))[0, 0]
        #         )
        #     except:
        #         similarity.append(np.nan)
        # max_indices = [
        #     index for index, value in enumerate(similarity) if value == max(similarity)
        # ]
        max_len = len(X)

        # 先收集候选向量列表
        candidate_preds = [ind2.subtree_semantics[i] for i in types2[type_]]
        # 如果候选向量的长度不全等于 max_len，则对所有向量进行填充
        if any(np.atleast_1d(vec).shape[0] < max_len for vec in candidate_preds):
            semantics = np.stack([pad_vector(vec, max_len) for vec in candidate_preds])
        else:
            semantics = np.stack(candidate_preds)

        # 假设 types2[type_] 中所有的 y_pred 都是长度为 n 的向量
        # try:
        #     candidate_preds = np.stack([ind2.subtree_semantics[i] for i in types2[type_]])
        # except:
        #     max_len = len(y)
        #     candidate_preds = np.stack([
        #         pad_vector(ind2.subtree_semantics[i], max_len) for i in types2[type_]
        #     ])
        # 计算所有 candidate_preds 的范数

        norm_preds = np.linalg.norm(semantics, axis=1)

        norm_y = np.linalg.norm(y)

        # 计算点积
        dot_products = semantics @ y
        # 计算余弦相似度
        cosine_sims = dot_products / (norm_preds * norm_y)
        # 处理异常或零范数的情况
        cosine_sims = np.where(np.isnan(cosine_sims), -np.inf, cosine_sims)
        max_val = np.max(cosine_sims)
        max_indices = np.where(cosine_sims == max_val)[0].tolist()


        parent_a = copy.deepcopy(ind1)
        parent_b = copy.deepcopy(ind2)
        if max_indices == []:
            type_ = random.choice(list(common_types))
            ...
            ind1_height = ind1.height
            ind2_height = ind2.height
            if ind1_height > 10 and ind2_height > 10:
                child_a = parent_a
                child_b = parent_b
            elif ind1_height <= 10 and ind2_height > 10:
                child_a = ind1
                child_b = parent_b
            elif ind1_height > 10 and ind2_height <= 10:
                child_a = parent_a
            ...
            # NRMSE = math.sqrt(np.mean((y - y_pred) ** 2)) / (y.max() - y.min())
            # if NRMSE<NRMSE_best:
            #     NRMSE_best=NRMSE
            #     best_index=index
            #     R=True
            ...
            slice1 = ind1.searchSubtree(index1)
            slice2 = ind2.searchSubtree(best_index + 1)
            ind1[slice1], ind2[slice2] = ind2[slice2], ind1[slice1]
            ...
            # y_pred_a = quick_evaluate(ind1[slice1], pset, new_x)
            # y_pred_b = quick_evaluate(ind2[slice2], pset, new_x)
            #
            # SS=np.mean(np.abs(y_pred_a-y_pred_b))
            # if SS>0.0001 and SS<0.4:
            #     ind1[slice1], ind2[slice2] = ind2[slice2], ind1[slice1]
            #     return ind1, ind2
            # else:
            #     count=count+1

            # best_subtree= ind2[ind2.searchSubtree(best_index+1)]
            # ind1[slice1]= best_subtree
            # ind2=creator.Individual([best_subtree[0]])
            # for k in range(1, len(best_subtree)+1):
            #     ind2.extend(best_subtree[k:])
...
def SSC_mate(ind1, ind2, pset, X):
    ...
    if len(common_types) > 0:
        ...
        similarity = np.zeros((len(types1[type_]), len(types2[type_])))

        # for i in types1[type_]:
        #     for j in types2[type_]:
        #         # subtree_a = ind1[ind1.searchSubtree(i)]
        #         # subtree_b = ind2[ind2.searchSubtree(j)]
        #         # y_pred_a_ground_truth = quick_evaluate(subtree_a, pset, X)
        #         # y_pred_b_ground_truth = quick_evaluate(subtree_b, pset, X)
        #
        #         y_pred_a = ind1.subtree_semantics[i]
        #         y_pred_b = ind2.subtree_semantics[j]
        #         # assert np.allclose(y_pred_a, y_pred_a_ground_truth), (
        #         #     f"{y_pred_a[:10]} {y_pred_a_ground_truth[:10]}"
        #         # )
        #         # assert np.allclose(y_pred_b, y_pred_b_ground_truth), (
        #         #     f"{y_pred_b[:10]} {y_pred_b_ground_truth[:10]}"
        #         # )
        #         try:
        #             similarity[i - 1, j - 1] = cosine_similarity(
        #                 y_pred_a.reshape(-1, 1), y_pred_b.reshape(-1, 1)
        #             )[0, 0]
        #         except:
        #             similarity[i - 1, j - 1] = np.nan


        max_len = len(X)

        # 先收集候选向量列表
        candidate_semantics_a = [ind1.subtree_semantics[i] for i in types1[type_]]
        # 如果候选向量的长度不全等于 max_len，则对所有向量进行填充
        if any(np.atleast_1d(vec).shape[0] < max_len for vec in candidate_semantics_a):
            semantics_a = np.stack([pad_vector(vec, max_len) for vec in candidate_semantics_a])
        else:
            semantics_a = np.stack(candidate_semantics_a)

        candidate_semantics_b = [ind2.subtree_semantics[j] for j in types2[type_]]
        if any(np.atleast_1d(vec).shape[0] < max_len for vec in candidate_semantics_b):
            semantics_b = np.stack([pad_vector(vec, max_len) for vec in candidate_semantics_b])
        else:
            semantics_b = np.stack(candidate_semantics_b)
        # 计算向量范数，并防止除零错误（0除以0可能引发异常）
        norm_a = np.linalg.norm(semantics_a, axis=1, keepdims=True)

        norm_b = np.linalg.norm(semantics_b, axis=1, keepdims=True)

        norm_a[norm_a == 0] = 1e-10
        norm_b[norm_b == 0] = 1e-10

        # 计算点积
        dot_products = semantics_a @ semantics_b.T

        # 得到余弦相似度矩阵
        similarity = dot_products / (norm_a * norm_b.T)

        max_indices = np.argwhere(similarity == np.nanmax(similarity))
        parent_a = copy.deepcopy(ind1)
        parent_b = copy.deepcopy(ind2)
        ...
        if max_indices.size == 0:
            ...
            slice1 = ind1.searchSubtree(index1 + 1)
            slice2 = ind2.searchSubtree(index2 + 1)
...
def staticLimit_multi(key, max_value):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if new_inds[0].height > max_value:
                new_inds[0] = random.choice(keep_inds[:2])
            if new_inds[1].height > max_value:
                new_inds[1] = random.choice(keep_inds[:2])
            return new_inds
